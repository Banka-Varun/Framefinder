...
async function handle(req:Request,ctx:{params:Promise<{path:string[]}>}){try{const {path}=await ctx.params;let result:Response;
 ...
 else{originCheck(req);if(path[0]==='auth'){const read=['session','config','google','google-callback'].includes(path[1]);if(req.method!==(read?'GET':'POST'))throw new ApiError(405,'Method not allowed');result=await auth(req,path[1]);}else{const readOnly=['members','movies','state','recommendations','notifications'];if(['state','recommendations'].includes(path[0])&&req.method!=='GET')throw new ApiError(405,'Method not allowed');if(path[0]==='members'&&!path[2]&&req.method!=='GET')throw new ApiError(405,'Method not allowed');if(path[0]==='movies'&&(!path[1]||path[2]==='providers')&&req.method!=='GET')throw new ApiError(405,'Method not allowed');result=await (path[0]==='admin'?admin(req,path):path[0]==='notifications'?notifications(req,path):path[0]==='posters'?posterResponse(req):path[0]==='seat-alerts'?seatAlerts(req,path):path[0]==='conversations'?chat(req,path):['billing','alerts','tickets'].includes(path[0])?commerce(req,path):social(req,path));}}
 result.headers.set('Cache-Control',path[0]==='posters'?'public, max-age=3600':'no-store');result.headers.set('X-Content-Type-Options','nosniff');return result;
 }catch(e){if(new URL(req.url).pathname==='/api/v1/auth/google-callback'){const message=e instanceof ApiError?e.message:'Google sign-in could not be completed. Please try again.';return new Response(null,{status:302,headers:{Location:'/login?authError='+encodeURIComponent(message),'Cache-Control':'no-store'}});}if(e instanceof ApiError)return Response.json({error:e.message},{status:e.status});if(e instanceof ZodError)return Response.json({error:e.issues[0]?.message||'Check your details.'},{status:400});console.error('Request failed',e instanceof Error?e.name:'unknown');return Response.json({error:'The request could not be completed. Please try again.'},{status:500});}}
