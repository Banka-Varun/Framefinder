...
Manage Subscription saves Free / Plus / Unlimited / Pro Max plan preferences and monthly / quarterly / half-yearly / yearly periods. All periods display proposed standard and discounted launch-preview prices; struck-through amounts are not claimed as previous selling prices. Quarterly prices from the supplied reference are preserved. Paid selection is explicitly a preview: no charge, checkout, paid entitlement, or automatic renewal is created. Free update preferences remain functional. The subscription page separates the member's private ratings from consented, published community highlights.
...
The profile dropdown is the single menu entry point for My Subscription, alongside icon-labelled My Profile, My Tickets, Settings, admin access when applicable, and Sign out. Appearance remains in Settings. Account confirmation uses a visible submit button and inline validation instead of a silently disabled action. Ticket notches are inward-facing and clipped inside the clickable card rectangle.
