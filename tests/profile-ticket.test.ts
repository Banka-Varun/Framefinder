...
assert.equal(other.user.avatar,'');
assert.equal(admin.user.avatar,'');
...
assert.equal((await one<any>('SELECT avatar FROM accounts WHERE id=?',[other.user.id])).avatar,'/api/v1/avatar?design=rose.deep.long.grin.glasses');
assert.equal((await(await call('auth/session','GET',undefined,admin.cookie)).json() as any).user.avatar,'');
